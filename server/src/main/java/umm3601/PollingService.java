...
class PollingService {
  ...
  private static final String machinesDatabaseName = "dev";

  PollingService(MongoClient mongoClient){
    this.mongoClient = mongoClient;
    MongoDatabase machinesDatabase = mongoClient.getDatabase(machinesDatabaseName);
    machinesCollection = machinesDatabase.getCollection("machines");
    this.poll();
  }
  ...
  private void updateAllMachineData(String responseData) {
    ...
    while (bsonValues.hasNext()) {
      ...
      machinesCollection.insertOne(d);
    }
  }
